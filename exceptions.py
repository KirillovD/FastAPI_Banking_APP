...
class IbanGenError(HTTPException):
    def __init__(
        self,
        detail: str = "Something went wrong, please refresh the page and try again",
    ):
        super().__init__(
...
class AccountOperationNotAllowed(HTTPException):
    def __init__(self, detail: str = "This operation is not allowed for this account type"):
        super().__init__(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=detail,
        )
...
class SelfTransferNotAllowed(HTTPException):
    def __init__(self, detail: str = "Source and recipient accounts must be different"):
        super().__init__(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=detail,
        )
...
class GraceNoInterest(HTTPException):
    def __init__(
        self,
        detail: str = "Can't add aquired interest to the balance: Grace period is active",
    ):
        super().__init__(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=detail,
